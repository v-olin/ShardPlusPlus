...
uniform float material_metalness = 0.f;
uniform float material_fresnel = 0.5f;
uniform float material_shininess = 2300.0f;
...
void main()
{
	vec3 texColor = normalize(vec3(40, 85, 156));

	vec3 wo = -normalize(viewSpacePosition);
	vec3 n = normalize(viewSpaceNormal);

	vec3 final = calculateDirectIllumiunation(wo, n, texColor) + calculateIndirectIllumination(wo, n, texColor);
